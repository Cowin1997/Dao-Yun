...
public class ParamService {
    ...
    public ResultModel<Boolean> addSysParam(SysParam param) {
        ...
        if (sysParam != null)
            return new ResultModel<>(ResultUtils.STATUS.ERROR, ResultUtils.MESSAGE.SYS_PARAM_KEY_EXIST, false);
    ...
    public ResultModel<Integer> addDict(DictParam param) {
        if (param.getSubs() == null) { //添加的是父级字典
            ...
            if (aBoolean)
                return new ResultModel<>(ResultUtils.STATUS.SUCCESS, ResultUtils.MESSAGE.ADD_SUCCESS, param.getId());
