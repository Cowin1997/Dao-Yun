...
public interface CheckTaskMapper {
    ...
    @Insert("insert into checktask(class_id,create_time,password,time_limit,teacher_id,type,info,status,longitude,latitude) values(#{class_id}," +
            "#{create_time},#{password},#{time_limit},#{teacher_id},#{type},#{info},#{status},#{longitude},#{latitude});")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    public Integer addTask(CheckTask task);

    @Update("update checktask set status=0 where id=#{id};")
    ...
    @Select("select * from checktask where status=1 and class_id=#{cid} order by create_time desc limit 1;")
