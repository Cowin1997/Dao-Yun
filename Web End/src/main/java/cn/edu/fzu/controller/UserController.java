...
public class UserController {
    ...
    public ResponseEntity<ResultModel<LoginRes>> userLogin(@RequestBody LoginReq loginReq, HttpServletRequest request) {
        ...
        if (loginReq.getUsername() != null) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ResultUtils.error(null, ResultUtils.MESSAGE.BAD_REQUEST));
    }
    ...
    public PageResult<User> getUser( @RequestParam(value = "page", required = true) Integer page,
                                                  @RequestParam(value = "size", required = true) Integer size){
        if(page<0 && size<0) throw new RuntimeException();
        PageResult<User> userPage = userService.getUserPage(page, size);
        return userPage;
    }
    ...
    public ResultModel<Boolean> addUser(@RequestBody User user){
        user.setUs_registtime(new Date());
        Boolean aBoolean = this.userMapper.addUser(user);
        if(aBoolean) return ResultUtils.success(true,"添加成功");
        return ResultUtils.error(true,"添加失败");
    }
