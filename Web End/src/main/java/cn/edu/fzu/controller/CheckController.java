...
public class CheckController {
    ...
    public ResultModel<Boolean> check(checkReq req) {
        CheckLog log = this.checkLogMapper.getLog(req.getTaskid(), req.getChecksid());
        if (log != null) return new ResultModel<>(ResultUtils.STATUS.ERROR, ResultUtils.MESSAGE.CHECK_ALREADY, false);
        else if (this.checkTaskMapper.isEnd(req.getTaskid()) == false)
            return new ResultModel<>(ResultUtils.STATUS.ERROR, ResultUtils.MESSAGE.CHECK_HAD_END, false);
        else {
            req.setChecktime(new Date());
            req.setCheckscore(2); //应该从系统参数读取每次签到的经验值
