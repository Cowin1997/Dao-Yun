...
public class DataConfigController {
    ...
    public Result addSysParam(@RequestBody SysParamDO sysParam){
        Boolean aBoolean = this.sysParamService.addSysParam(sysParam);
        if(aBoolean)
            return Result.success(ResultCodeEnum.SUCCESS,sysParam.getId());
        return Result.failure(ResultCodeEnum.FAILURE);
    }
    ...
    @ResponseBody
    @ApiOperation(value = "deleteDictById",notes = "根据ID删除字典(同时删除子字典项）")
    ...
    @DeleteMapping(value = "/sub-dict/{id}")
    @ResponseBody
    @ApiOperation(value = "deleteDictById",notes = "根据ID删除字典(同时删除子字典项）")
    public Result<Boolean> deletesubSubDictById(@PathVariable(value = "id") Integer id){
        Boolean aBoolean = this.sysDictService.deleteSubDict(id);
        return Result.success(ResultCodeEnum.SUCCESS,aBoolean);
    }
